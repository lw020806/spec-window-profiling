...
void* do_work(void* args) {
    ...
    while (node < N) {
        ...
        for (v = 0; v < N; v++) {
            // bounds check v + dist < N
            // prefetch &W_index[v + dist][0] at (70:1) (122:1)
            for (int i = 0; i < DEG; i++) {
                if (W_index[v][i] != -1) {
...
// Input arguments
int main_orig(const int P1, const int N, const int DEG) {
    int P = 256;
    start = P1;
    P = P1;
...
int main(int argc, char** argv) {
    if (argc == 2) {  // fuzzing mode : fuzzers usually prefer to get input from a file
        FILE* fp;
...
#define MAX_LEN 1024
        ...
        if (data_len >= MAX_LEN || data_len <= MIN_LEN) {
            return 0;
        }
        ...
        int n = (data[0] | data[1] << 8 | data[2] << 16 | data[3] << 24) % 512;
        int deg = (data[4] | data[5] << 8 | data[6] << 16) % 32;

        main_orig(1, n, deg);
    } else if (argc == 4) {  // profiling mode
        const int P1 = atoi(argv[1]);
        const int N = atoi(argv[2]);
        const int DEG = atoi(argv[3]);
        main_orig(P1, N, DEG);
    } else {  // wrong cmdline arguments
        printf("Usage: a) ./fuzz <input file name> b) ./bc_sf P N DEG\n");
        return 1;
    }
